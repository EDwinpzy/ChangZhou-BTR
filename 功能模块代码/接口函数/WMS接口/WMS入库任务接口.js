 ...
 * @LastEditors: EDwin
 * @LastEditTime: 2022-01-25 15:39:07
 */
/**
...
function WMS_outTask(inInfo) {
    ...
    try {
        ...
        //查出入库任务表中的入库任务
        var WMS_outstore_order = $Function.toDataSet($System.BTR, `SELECT * FROM ${dataBase[0]} WHERE Package_Code IN (${jobIDS_arr.join(',')}) AND in_or _out = 1`); //查询该小批次的入库任务
        if (!WMS_outstore_order) throw 'WMS入库指令接口表查询失败！';
        //查生产工单历史表get_history
        var get_history = $Function.toDataSet($System.BTR, `SELECT * FROM ${dataBase[3]} WHERE jobID IN (${jobIDS_arr.join(',')})`); //查询该小批次的入库任务
        if (!get_history) throw '该小批次' + jobIDS_arr + '未生产完成！无法入库！';
        get_history = $Function.toMap(['jobIDS'], get_history); //转换成字典
        var existOrder = [];
        inInfo.forEach(function (item) {
        ...
        var res = $Function.SqlInsert(inOrder, dataBase[0]);
        ...
        if (existOrder.length > 0) {
            existOrder.forEach(function (item) {
                $Function.tip('warning', '小批次号为：' + item.jobIDS + '的物料已存在入库任务，入库单号为：' + item.taskID);
            });
        }
    }
}
